class Quotation < ApplicationRecord
  ...
  def self.string_as_keys(integer)
    if integer == 1
      "january"
    elsif integer == 2
      "february"
  ...
  def self.sql_query(year, classname, param)
    @queries = {}
    @month_string_keys = {}
    ...
    @queries[:group_by_month_and_category].each do |key, value|
      @month_and_category_string_keys[[key[0],classname.capitalize.constantize.string_as_keys(key[-1].to_i)]] = value
    end
    ...
    @sorted_months.each do |month_array|
      @test_hash[classname.capitalize.constantize.string_as_keys(month_array[0])] = month_array[-1]
    end
